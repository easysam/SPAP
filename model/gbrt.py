...
def evaluate(logger, model, data_loaders, batch_size):
    ...
    for test_cx, test_ex, test_t, test_y in data_loaders['test']:
        ...
        y = test_y[:,-1].float().numpy()

        ratio_output = model.predict(np.hstack((cx, ex)))
...
def train_cdp(logger, model_name, model, data_loaders, train_mode, batch_size, model_file_name, epoch, device, train_progress):
    ...
    for i in range(epoch):
        ...
        all_t = []
        all_y = []
        size = len(data_loaders)
        for j, (train_cx, train_ex, train_t, train_y) in enumerate(data_loaders['train']):
            if j == size // 5:
                break
            all_cx.append(train_cx.view(batch_size, -1).float().numpy())
            all_ex.append(train_ex.view(batch_size, -1).float().numpy())
            all_y.append(train_y[:,-1].float().numpy())

        CX = np.vstack(all_cx)
        EX = np.vstack(all_ex)
        # T = np.vstack(all_t)
        Y = np.vstack(all_y)
        model.fit(np.hstack([CX, EX]), Y.ravel())
        rank_metric = evaluate(logger, model, data_loaders, batch_size)
        logger.info('mode {} repeat {}/{} train progress {}/{} epoch {}/{} test  pred loss: {:.4f} map@10: {:.4f}'
                    .format(train_mode,
        ...
        logger.info('map@10: {:.4f} map@15: {:.4f} map@20: {:.4f} map@25: {:.4f} ndcg@10: {:.4f} ndcg@15: {:.4f} ndcg@20: {:.4f} ndcg@25: {:.4f}'
                    .format(rank_metric['map@10'], rank_metric['map@15'], rank_metric['map@20'], rank_metric['map@25'],
                            rank_metric['ndcg@10'], rank_metric['ndcg@15'], rank_metric['ndcg@20'], rank_metric['ndcg@25']))

        map_list.append(rank_metric)

        if best_map_metric == None or rank_metric['map@10'] > best_map_score:
    ...
    return sorted(map_list, key=lambda x: x['map@10'])[epoch // 6], sorted(map_list, key=lambda x: x['rmse'])[epoch // 2]
...
def one_pass_cdp(logger, model_name, source_city, target_city, train_mode,
    ...
    model_params = {'learning_rate': param['lr'], 'n_estimators': int(param['n_estimators']), 'max_depth': int(param['max_depth']), 'loss': 'ls'}
    model = GradientBoostingRegressor(**model_params)

    model_file_name = '{}_{}_{}_bs_{}_alpha_{}_neigh_{}' \
        .format(model_name, source_city, target_city, param['bs'], param['lr'], param['n_estimators'], param['max_depth'], param['neigh'])
...
def grid_search_cdp(logger, model_name, source_city="beijing", target_city="tianjing",
    ...
    header = {'lr': 5, 'n_estimators': 15, 'max_depth': 11, 'neigh': 5, 'bs': 5,
...
if __name__ == '__main__':
    ...
    logger = build_log('test', files['log']['model_log'] + os.sep + 'gbrt')
    logger.info('=' * 50)
    model_name = 'gbrt'
