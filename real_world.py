...
if __name__ == '__main__':
    ...
    result_path = 'result/rw_retrain_{}2{}.npy'.format(args.source, args.target)
    print('RESULT:', result_path)
    CO_t = np.array([[33, 54]] * candidate_num, dtype=int)
    P_t = np.array([[5.6, 48]] * candidate_num, dtype=float)
    budget = (real_C_t * CO_t).sum()
    print('BUDGET: {}, REVENUE: {}'.format(budget, revenue(real_C_t, Y_t, P_t)))
    for single_run in range(10):
        ...
        time_key_cnt = len(real_y) // candidate_nums[args.target]
        real_y = real_y.reshape(time_key_cnt, candidate_nums[args.target], -1).mean(axis=0)
        real_y = real_y[:, [0, 1]]
        real_R = revenue(real_C_t, real_y, P_t)
        result[3, single_run] = real_R
